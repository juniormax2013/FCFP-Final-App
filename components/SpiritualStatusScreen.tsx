...
import { ArrowLeft, Plus, Trash2, Edit3, X, Search, Flame, Check } from 'lucide-react';
import { Translation, SystemSettings } from '../types';

interface SpiritualStatusScreenProps {
  translation: Translation; settings: SystemSettings; statuses: string[]; onBack: () => void; onAddStatus: (s: string) => void; onUpdateStatus: (old: string, updated: string) => void; onDeleteStatus: (s: string) => void;
}

export const SpiritualStatusScreen: React.FC<SpiritualStatusScreenProps> = ({ translation, settings, statuses, onBack, onAddStatus, onUpdateStatus, onDeleteStatus }) => {
  ...
  const handleAdd = () => { if (newVal.trim()) { onAddStatus(newVal.trim()); setNewVal(''); setIsAdding(false); } };
  const handleUpdate = () => { if (editing && editText.trim()) { onUpdateStatus(editing, editText.trim()); setEditing(null); setEditText(''); } };
    ...
    <div className={`h-screen w-full flex flex-col overflow-y-auto pb-32 ${isLight ? 'bg-white text-slate-900' : 'bg-black text-white'}`}>
      <div className={`w-full max-w-2xl mx-auto px-4 py-6 flex items-center justify-between sticky top-0 z-30 backdrop-blur-md ${isLight ? 'bg-white/80 border-b border-slate-200' : 'bg-black/80'}`}>
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold">{translation.spiritualStatus}</h2>
        </div>
        <button onClick={() => setIsAdding(true)} className="p-2 bg-amber-500 rounded-full text-white shadow-lg active:scale-95 transition-all"><Plus size={24} /></button>
      </div>
      <div className="w-full max-w-2xl mx-auto px-4 mt-4">
        ...
        {isAdding && (
          <div className={`rounded-[32px] p-6 mb-8 border animate-in slide-in-from-top-4 ${isLight ? 'bg-white border-amber-200 shadow-xl' : 'glass border-amber-500/30'}`}>
            <div className="flex items-center gap-4">
              <input autoFocus type="text" placeholder={translation.spiritualStatus} value={newVal} onChange={e => setNewVal(e.target.value)} className={`flex-1 border rounded-2xl px-4 py-3 text-sm focus:outline-none ${isLight ? 'bg-slate-50 border-slate-200 text-slate-900' : 'bg-white/5 border-white/10 text-white'}`} />
              <button onClick={handleAdd} className="p-3 bg-amber-500 rounded-2xl text-white"><Check size={20} /></button>
        ...
        <div className="space-y-4">
          {filtered.map((item) => (
            <div key={item} className={`rounded-[28px] p-6 flex items-center justify-between border transition-all ${isLight ? 'bg-white border-slate-100 shadow-sm hover:border-amber-200' : 'glass border-white/5 hover:bg-white/10'}`}>
              <div className="flex items-center gap-4 flex-1">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isLight ? 'bg-amber-50 text-amber-500' : 'bg-amber-500/20 text-amber-500'}`}><Flame size={20} /></div>
                {editing === item ? (
